# Filename: CitadelV4_1_Strategy.py
...
class CitadelV4_1_Strategy(IStrategy):
    """
    Citadel System - Version 4.1 - Multi-Lens Sniper Entry
    
    This strategy represents a major evolution, combining multiple timeframes
    for a complete trading system, from macro trend to micro entry.
    
    V4.1 Layers:
    1.  4H MACRO LENS: Identifies the overall market "weather" (Strong/Weak Bull).
    2.  1H MESO LENS: Identifies a "Zone of Interest" using SMC and Volume.
    3.  15m MICRO LENS: Executes a "Sniper Entry" on a precise BOS+FVG pattern.
    4.  DYNAMIC RISK: Stop-loss is now based on market structure, and position
        size is calculated based on the dollar-risk to that structural low.
    """
    ...
    # --- EXIT MECHANISM (from V3) ---
    use_custom_stoploss = True
    minimal_roi = {"0": 0.042}
    use_exit_signal = False

    # --- V3 OPTIMIZED PARAMETERS ---
    bbw_squeeze_threshold = 0.42
    adx_threshold = 27
    volume_lookback = 70
    volume_zscore_threshold = 0.8
    base_risk_allocation = 0.029
    weak_bull_modifier = 0.8
    tight_trailing_stop_pct = 0.017
    
    # --- V4.1 NEW PARAMETERS ---
    bos_lookback_candles = 10
    max_structural_stop_pct = 0.062
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Initialize the dynamic_stop_loss_pct column with a default value (e.g., NaN)
        dataframe['dynamic_stop_loss_pct'] = np.nan
        
        # Calculate the potential stop distance for all candles
        ...
        # Set the columns separately and safely
